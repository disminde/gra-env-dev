...
功能:
    1. 执行 QM 偏差校正
    2. 直接写入数据库表 qm_corrected_grid_data
    3. 保留 CSV 备份（可选）
    4. 支持 ET0 校正扩展（预留）

输出:
    - 数据库表：qm_corrected_grid_data
    - CSV 文件：scripts/data/processed/qm_correction/（备份）
    - QM 模型：scripts/data/processed/qm_correction/qm_models/
"""
...
def main():
    ...
    logger.info("="*80)
    logger.info("华北平原气象数据 QM 偏差校正系统 (数据库版本)")
    logger.info("="*80)
    ...
    logger.info(f"  数据库写入：{CONFIG['database']['enabled']}")
    logger.info(f"  目标表：{CONFIG['database']['table_name']}")
    ...
    if CONFIG['database']['enabled']:
        logger.info(f"  ✓ 数据库表：{CONFIG['database']['table_name']}")
        logger.info(f"    - 记录数：约 {68 * 34 * 365:,} 条")
        logger.info(f"    - 包含字段：校正后的温度、降水、风速、相对湿度")
    
    if CONFIG['database']['backup_csv']:
        logger.info(f"  ✓ CSV 备份：{CONFIG['output_dir']}/*_corrected.csv")
