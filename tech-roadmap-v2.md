...
鉴于此，本研究提出一种融合多源数据的技术路径：以Open-Meteo API提供的ERA5-Land再分析数据为主力数据源，以保证空间连续性；同时利用NOAA（National Oceanic and Atmospheric Administration）获取的地面气象站实测数据（SYNOP/METAR）作为“真值”基准，构建偏差校正模型。这种“格点+站点”的融合策略，旨在兼顾数据的空间覆盖度与物理真实性。
...
1.  **多源数据工程**：构建基于Open-Meteo API的自动化数据采集流水线，并结合NOAA实测数据开发基于分位数映射的偏差校正模块。
...
*   **NOAA 气象数据库**：提供北京、天津、石家庄、济南、郑州等关键站点的SYNOP/METAR报文数据，作为偏差校正的参考真值。
...
#### 3.1.2 基于NOAA的分位数映射（QM）偏差校正
再分析数据虽然覆盖面广，但往往存在系统性偏差。本设计采用分位数映射法（Quantile Mapping, QM），这是一种非参数偏差校正方法，旨在使模拟数据（ERA5）的累积分布函数（CDF）与观测数据（NOAA）的CDF相匹配。

**校正算法步骤：**
1.  **时空匹配（Spatial Matching）**：
    首先，需要将格点数据映射到站点位置。虽然最简单的方法是最近邻法（Nearest Neighbor），但为了提高精度，本系统采用**双线性插值（Bilinear Interpolation）**。对于任意NOAA站点 $S$，选取包围该站点的四个Open-Meteo格点 $G_1, G_2, G_3, G_4$，根据距离权重计算该位置的模拟值 $M_{raw}$。
2.  **构建经验累积分布函数（eCDF）**：
...
### 3.7 实时推演流水线设计 (Live Inference Pipeline)

在实际业务场景中，为了预测当前（如2026年）的最新干旱状态，系统需要持续获取最新气象数据。考虑到 Open-Meteo 开放 API 的访问频次限制，系统设计了前后端解耦的“实时推演流水线”：

1. **降维获取与聚合**：彻底放弃极其消耗 API 额度的小时级数据，转而请求**日级 (Daily)** 分辨率数据（将请求量降低至原来的 1/24）。
2. **离线更新机制**：编写独立的离线脚本 (`update_latest_data.py`)，定期（如每日或每周）拉取最新的日级数据，并在本地进行特征工程处理，将其聚合为模型所需的**月度特征 (Monthly Features)**（包括滞后特征和滑动窗口统计量），最终保存为轻量级的本地数据文件。
3. **前端解耦推演**：Streamlit 前端应用 (`streamlit_app.py`) 不再直接与外部 API 交互，而是直接读取本地已更新的月度特征文件。这不仅彻底规避了用户在界面交互时触发 API 限流的风险，还极大提升了 SHAP 归因分析和 What-If 情景推演的响应速度。
4. **模型架构回归**：鉴于深度学习模型（ConvLSTM）在月度空间张量聚合后样本量锐减（仅剩约308个时间步张量），难以充分拟合物理规律，实时推演系统最终选用在结构化表格数据（约305万条样本）上表现更优且具备完全可解释性的 **XGBoost** 模型作为核心推理引擎。

## 第四章 预期成果与可行性分析

### 4.1 预期成果
1.  **高精度数据集**：构建一套覆盖华北平原近40年、经NOAA实测数据校正的格点化气象与干旱指数数据集，为后续研究提供数据基础。
...
### 4.2 可行性分析
*   **数据获取可行性**：Open-Meteo API免费且无需鉴权，NOAA数据公开可得，两者结合的技术路径在气象学界已有成熟应用，不存在法律与技术壁垒。
