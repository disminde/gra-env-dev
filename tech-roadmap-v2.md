...
![该架构图展示了从Open-Meteo和RP5获取原始数据，经偏差校正模块处理后存入数据库，供LSTM/XGBoost/RF模型训练，并通过SHAP进行解释，最终在Docker容器化的Streamlit应用中展示的完整流程。](md-pic\pic-1.jpg)
...
![该图展示了偏差校正模块的工作原理：首先通过双线性插值将Open-Meteo格点数据匹配至RP5站点位置；接着构建两者的累积分布函数(CDF)；通过映射函数纠正模拟数据的分布偏差；最后将校正参数空间插值回整个华北平原网格。](md-pic\pic-2.jpg)
...
![左图：LSTM的时间展开结构，展示记忆单元与门控机制如何处理序列输入；中图：XGBoost的加法模型结构，展示残差学习过程；右图：随机森林的并行集成结构，展示多棵决策树的投票机制。](md-pic\pic-3.jpg)
...
![该图展示了各气象因子对SPEI预测值的SHAP影响分布。Y轴为特征排序（按重要性递减），X轴为SHAP值（负值表示促使SPEI降低，即加剧干旱）。点的颜色代表特征值的大小（红色为高值，蓝色为低值）。例如，'Temperature_Lag1'行右侧的红色点群表明，上个月的高温倾向于降低本月的SPEI值（加剧干旱）。Data sources: Natural Hazards 2025, Atmosphere 2024, NHESS 2025](md-pic\pic-4.png)
...
![该界面展示了“情景推演”模块。左侧侧边栏提供“气温增幅”和“降水变率”调节滑块。右侧主视图实时显示在当前假设情景下，未来6个月的SPEI预测曲线（红色虚线）与基准情景（蓝色实线）的对比，以及干旱风险的空间分布变化。](md-pic\pic-5.jpg) 
