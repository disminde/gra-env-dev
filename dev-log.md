# 学习与开发日志

**使用说明**：本日志用于记录毕业设计过程中的学习心得、开发进展、遇到的技术难题及解决方案。建议每周至少更新一次，保持记录的连贯性。

## 📝 模板范例

### [202X-XX-XX] 第X周 - [阶段主题]

**📅 本周概览**
- **核心目标**：完成Open-Meteo数据接口的调试。
- **完成情况**：100% / 延迟 / 提前。
- **心情/状态**：⭐⭐⭐⭐ (4/5)

**📚 学习内容**
1.  阅读了Open-Meteo官方文档，理解了API请求参数的含义。
2.  学习了Python `requests` 库的高级用法（重试机制、超时处理）。
3.  复习了Pandas的时间序列处理功能 (`resample`, `rolling`)。

**💻 开发进展**
- [x] 初始化了Git仓库并连接GitHub。
- [x] 编写了 `data_fetcher.py` 脚本，可以成功拉取指定经纬度的气温数据。
- [ ] 数据库表结构设计尚未完成，还在对比TimescaleDB和InfluxDB。

**🐛 问题与解决**
- **问题1**：API请求频率限制导致连接中断。
    - **原因**：短时间内发送了过多请求。
    - **解决**：在循环中加入了 `time.sleep(1)`，并使用了 `requests.Session()` 复用连接。
- **问题2**：下载的数据格式是JSON，嵌套层级较深，解析困难。
    - **解决**：编写了一个递归函数提取数据，最后转换为DataFrame。

**🚀 下周计划**
- 完成数据库环境搭建。
- 实现数据入库功能。

---

## 🗓️ 日志记录

### [2026-01-25] 项目启动阶段

**📅 本周概览**
- **核心目标**：项目环境初始化，完善技术文档，制定详细开发计划。
- **完成情况**：进行中。

**💻 开发进展**
- [x] 基于 `tech-roadmap-v2.md` 完成了项目技术路线的确认。
- [x] 初始化了本地 Git 仓库，配置了 `.gitignore`。
- [x] 关联了远程 GitHub 仓库 `disminde/gra-env-dev`。
- [x] 创建了 `README.md`、`project-plan.md` 和 `dev-log.md` 等基础文档。

**📝 备注**
- 今天的重点是把基础设施搭好，接下来的重点是学习 Streamlit 和 气象数据处理的基础知识。
